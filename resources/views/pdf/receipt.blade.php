...
<body>
	...
	<div style="margin-top: 25px;">
		...
		<div >
			...
			<div style="width: 79%; border-bottom: 0.5px solid black; margin-top: -29px; margin-left: 140px;">@if(count($payment->account->enrollee)>0){{$payment->account->enrollee->firstName. ' ' . $payment->account->enrollee->middleName . ' ' .$payment->account->enrollee->lastName}}@elseif(count($payment->account->groupapplication)>0) {{$payment->account->groupapplication->orgName}}  @endif</div>
			<div style="position: relative;"><label class="control-label col-md-2" > and address at</label></div>
			<div style="width: 79.25%; border-bottom: 0.5px solid black; position: relative; top: -29px; margin-left: 19%;">@if(count($payment->account->enrollee)>0) {{$payment->account->enrollee->street. ' ' .$payment->account->enrollee->barangay . ' ' . $payment->account->enrollee->city}} @elseif(count($payment->account->groupapplication)>0) {{$payment->account->groupapplication->orgAddress}} @endif</div>
