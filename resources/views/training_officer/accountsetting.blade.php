                                        ...
                                        <div class="col-md-4">
					                        <div class="fileinput fileinput-new" data-provides="fileinput">
					                            <div class="fileinput-new thumbnail img-file">
					                                <img src="/display_image/{{Auth::user()->trainingofficer->image}}" width="300px" height="250px" alt="..."></div>
					                            <div class="fileinput-preview fileinput-exists thumbnail img-max"></div>
					                            <div>
                                                    <form id="uploadForm" action="/tofficer/update-image" method="POST" enctype="multipart/form-data">
                                                        {{csrf_field()}}
                                            ...
                                            <div class="table-responsive">
                                                <table class="table table-bordered table-striped" id="users">
                                                    <tr>
                                                        ...
                                                        <td width="55%" id="fieldFN">{{Auth::user()->trainingofficer->firstName . ' ' . Auth::user()->trainingofficer->middleName . ' ' . Auth::user()->trainingofficer->lastName}}</td>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td width="55%" id="fieldAD">{{Auth::user()->trainingofficer->street . ' ' . Auth::user()->trainingofficer->barangay . ' ' . Auth::user()->trainingofficer->city}}</td>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td width="55%" id="fieldDOB">{{Carbon\Carbon::parse(Auth::user()->trainingofficer->dob)->format('F d, Y')}}</td>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td width="55%" id="fieldCO">{{Auth::user()->trainingofficer->contact}}</td>
...
<script type="text/javascript">
    function update(name)
    {
        switch(name)
        {
            case 'FN':
                $('#field'+name).empty();
                $('#field'+name).append('<input type="text" id="firstName" class="form-control col-md-4" value="{{Auth::user()->trainingofficer->firstName}}"><input type="text" id="middleName" class="form-control col-md-4" value="{{Auth::user()->trainingofficer->middleName}}"><input type="text" id="lastName" class="form-control col-md-4" value="{{Auth::user()->trainingofficer->lastName}}">');
                $('#button'+name).empty();
            ...
            case 'AD':
                $('#field'+name).empty();
                $('#field'+name).append('<input type="text" id="street" class="form-control col-md-4" value="{{Auth::user()->trainingofficer->street}}"><input type="text" id="barangay" class="form-control col-md-4" value="{{Auth::user()->trainingofficer->barangay}}"><input type="text" id="city" class="form-control col-md-4" value="{{Auth::user()->trainingofficer->city}}">');
                $('#button'+name).empty();
                $('#button'+name).append('<button onclick="save('+"'"+name+"'"+')" type="button" class="btn btn-primary btn-sm"><i class="glyphicon glyphicon-ok"></i></button>&ensp;<button onclick="remove('+"'"+name+"'"+')" type="button" class="btn btn-danger btn-sm"><i class="glyphicon glyphicon-remove"></i></button>');
                break;
            case 'DOB': 
                $('#field'+name).empty();
                $('#field'+name).append('<div class="input-group date form_datetime" data-link-field="dtp_input1"><input class="form-control hasDatepicker" size="16" type="text" value="{{Carbon\Carbon::parse(Auth::user()->trainingofficer->dob)->format("F d,Y")}}" id="dob" name="dob" readonly><span class="input-group-addon"><span class="glyphicon glyphicon-th"></span></span></div>');
                $('#button'+name).empty();
                $('#button'+name).append('<button onclick="save('+"'"+name+"'"+')" type="button" class="btn btn-primary btn-sm"><i class="glyphicon glyphicon-ok"></i></button>&ensp;<button onclick="remove('+"'"+name+"'"+')" type="button" class="btn btn-danger btn-sm"><i class="glyphicon glyphicon-remove"></i></button>');
            ...
            case 'CO': 
                $('#field'+name).empty();
                $('#field'+name).append('<div class="col-md-6"><select name="contactType" id="contactType" class="selector form-control" style="padding: 0px; width: 80%;" onchange="contactTypeChanged(this)">@if(strlen(Auth::user()->trainingofficer->contact)>8)<option selected value="mobile">Mobile No.</option><option value="tel">Landline No.</option>@else<option value="mobile">Mobile No.</option><option selected value="tel">Landline No.</option> @endif<select></div><div class="col-md-6" id="contactC" style="margin-left: -40px;"><input type="text" value="{{Auth::user()->trainingofficer->contact}}" id="contact" class="selector cp form-control placeholder" name="contact" placeholder="0999 9999 999"></div>');
                $('#button'+name).empty();
                $('#button'+name).append('<button onclick="save('+"'"+name+"'"+')" type="button" class="btn btn-primary btn-sm"><i class="glyphicon glyphicon-ok"></i></button>&ensp;<button onclick="remove('+"'"+name+"'"+')" type="button" class="btn btn-danger btn-sm"><i class="glyphicon glyphicon-remove"></i></button>');
                $('.tel').mask('000 0000',
              {
    ...
    function remove(name){
        switch(name)
        {
            case 'FN':
                $('#field'+name).empty();
                $('#field'+name).append('{{Auth::user()->trainingofficer->firstName . ' ' . Auth::user()->trainingofficer->middleName . ' ' . Auth::user()->trainingofficer->lastName}}');
                $('#button'+name).empty();
            ...
            case 'AD':
                $('#field'+name).empty();
                $('#field'+name).append('{{Auth::user()->trainingofficer->street . ' ' . Auth::user()->trainingofficer->barangay . ' ' . Auth::user()->trainingofficer->city}}');
                $('#button'+name).empty();
                $('#button'+name).append('<button onclick="update('+"'"+name+"'"+')" type="button" class="btn btn-primary btn-sm"><i class="glyphicon glyphicon-pencil"></i></button>');
                break;
            case 'DOB':
                $('#field'+name).empty();
                $('#field'+name).append('{{Carbon\Carbon::parse(Auth::user()->trainingofficer->dob)->format("F d, Y")}}');
                $('#button'+name).empty();
                $('#button'+name).append('<button onclick="update('+"'"+name+"'"+')" type="button" class="btn btn-primary btn-sm"><i class="glyphicon glyphicon-pencil"></i></button>');
                break;
            case 'CO': 
                $('#field'+name).empty();
                $('#field'+name).append('{{Auth::user()->trainingofficer->contact}}');
                $('#button'+name).empty();
                $('#button'+name).append('<button onclick="update('+"'"+name+"'"+')" type="button" class="btn btn-primary btn-sm"><i class="glyphicon glyphicon-pencil"></i></button>');
                break;
            case 'EM': 
    ...
    function save(name){
        switch(name)
        {
            case 'FN':
                ...
                $.ajax({
                    type:'get',
                    url:'{!!URL::to('/tofficer/update-firstname')!!}',
            ...
            case 'AD':
                ...
                $.ajax({
                    type:'get',
                    url:'{!!URL::to('/tofficer/update-address')!!}',
            ...
            case 'EM':
                ...
                $.ajax({
                    type:'get',
                    url:'{!!URL::to('/tofficer/update-email')!!}',
                    data:{'email':email},
                    success:function(data){
                        $('#field'+name).empty();
            ...
            case 'DOB':
                ...
                $.ajax({
                    type:'get',
                    url:'{!!URL::to('/tofficer/update-dob')!!}',
                    data:{'dob':dob},
                    success:function(data){
                        $('#field'+name).empty();
            ...
            case 'CO':
                ...
                $.ajax({
                    type:'get',
                    url:'{!!URL::to('/tofficer/update-contact')!!}',
    ...
    function contactTypeChanged(contactType){
      if(contactType.value == "tel"){
        $("#contactC").empty();
        @if(strlen(Auth::user()->trainingofficer->contact)<=8)
        $("#contactC").append('<input type="text" id="contact" value="{{Auth::user()->trainingofficer->contact}}" name="contact" class="tel form-control placeholder" placeholder="999 9999">')
        @else
        ...
        $("#contactC").empty();
        @if(strlen(Auth::user()->trainingofficer->contact)>8)
        $("#contactC").append('<input type="text" id="contact" value="{{Auth::user()->trainingofficer->contact}}" name="contact" class="cp form-control placeholder" placeholder="0999 9999 999">')
        @else
